...
/**
 * Use the current classloader to read all plugin define file. The file must be named 'JVMAop-plugin.def'
 */
public class PluginResourcesResolver {
    ...
    public List<URL> getResources() {
        ...
        try {
            urls = AgentClassLoader.getDefault().getResources("JVMAop-plugin.def");

            while (urls.hasMoreElements()) {
                URL pluginUrl = urls.nextElement();
                cfgUrlPaths.add(pluginUrl);
                LOGGER.info("find JVMAop plugin define in {}", pluginUrl);
            }
